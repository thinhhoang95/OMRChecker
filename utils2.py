...
def handle_markers(image_norm, marker, curr_filename):
    ...
    h, w = optimal_marker.shape[0], optimal_marker.shape[1]
    centres = []
    sumT, maxT = 0, 0
...
def readResponse(template, image, name, savedir=None, autoAlign=False):
    ...
    try:
        ...
        for QBlock in template.QBlocks:
            QStdVals = []
            for qStrip, qBoxPts in QBlock.traverse_pts: # columns of each QBlock
                QStripvals = []
                for pt in qBoxPts: # rows of each QBlock
                    # shifted
        ...
        for QBlock in template.QBlocks:
            ...
            for qStrip, qBoxPts in QBlock.traverse_pts:
                ...
                if(config.showimglvl >= 5): # set a breakpoint here!
